...
// Stage 1 of the React rewrite: a Vite/React client served at /v2, proven here against the
// SAME testid contract as the legacy front end at / (test-e2e/auth.spec.js) — reusing those
// constants unchanged is the proof the contract is genuinely front-end agnostic.

test.use({ storageState: { cookies: [], origins: [] } });

test('logged-out state renders the login screen at /v2', async ({ page }) => {
  await page.goto('/v2/');
